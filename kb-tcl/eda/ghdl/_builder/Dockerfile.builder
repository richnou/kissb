...
RUN dnf install -y --enablerepo=devel gcc-gnat diffutils
RUN dnf install -y --enablerepo=devel llvm-toolset llvm-devel
RUN dnf install -y --enablerepo=devel zlib-devel
